...
function part2(ascii_str::Vector{UInt8})
    # boxes = Vector{OrderedDict}(undef, 256)
    ...
    while i <= length(ascii_str)
        ...
        # if @inbounds !isassigned(boxes, idx+1)
        #     boxes[idx+1] = OrderedDict()
        # end

        val = @inbounds @views ascii_str[i+1] - '0'u8

        label = @inbounds ascii_str[start_i:i-1]
        if c == '='u8
