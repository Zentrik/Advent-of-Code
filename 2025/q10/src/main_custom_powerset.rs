...
fn get_all_p1_sols(
    ...
    // Iterate over power set of buttons
    let num_buttons = buttons_bitmask.len();
    for powerset_bitmask in 0..(1u16 << num_buttons) {
        let mut indicator_lights = 0u16;
        for (i, &button) in buttons_bitmask.iter().enumerate() {
            let button_toggled = (powerset_bitmask & (1 << i)) != 0;
            if button_toggled {
                indicator_lights ^= button;
            }
        }
        ...
        if indicator_lights == target_lights_bitmask {
            all_solutions.push(powerset_bitmask);
        }
    }
...
fn get_subproblem_joltage(
    ...
    buttons_bitmask: &Vec<u16>,
    toggled_button_bitmask: u16,
) -> bool{
    for (i, &button) in buttons_bitmask.iter().enumerate() {
        if (toggled_button_bitmask & (1 << i)) == 0 {
            continue;
        }
...
fn solve_p2(buttons_bitmask: &Vec<u16>, target_joltage: &Vec<u16>) -> u16 {
    ...
    for mod2_sol in get_all_p1_sols(buttons_bitmask, subtarget_joltage_bitmask) {
        ...
        if !get_subproblem_joltage(&mut mut_target_joltage, buttons_bitmask, mod2_sol) {
            continue;
        }

        let subproblem_soln = solve_p2(buttons_bitmask, &mut_target_joltage);
        if subproblem_soln == u16::MAX {
            continue;
        }
        p2_result = p2_result.min(2 * subproblem_soln + mod2_sol.count_ones() as u16);
    }
